...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers-1):
            if i==0:
                self.params['W'+str(i+1)] = np.random.normal(scale=weight_scale, size=(input_dim, hidden_dims[i])).astype(dtype)
                self.params['b'+str(i+1)] = np.zeros((hidden_dims[i],), dtype=dtype)
            else:
                self.params['W'+str(i+1)] = np.random.normal(scale=weight_scale, size=(hidden_dims[i-1], hidden_dims[i])).astype(dtype)
                self.params['b'+str(i+1)] = np.zeros((hidden_dims[i],), dtype=dtype)
            
            if self.normalization in ['batchnorm', 'layernorm']:
                self.params['gamma'+str(i+1)] = np.ones(hidden_dims[i], dtype=dtype)
                self.params['beta'+str(i+1)] = np.zeros(hidden_dims[i], dtype=dtype)
            
            self.params['W'+str(self.num_layers)] = np.random.normal(scale=weight_scale, size=(hidden_dims[-1], num_classes)).astype(dtype)
    ...
    def loss(self, X, y=None):
        ...
        dout, grads['W'+str(self.num_layers)], grads['b'+str(self.num_layers)] = affine_backward(dout, caches.pop())
        ...
        for i in range(self.num_layers-1):
            ...
            if self.normalization is None:
                dout, grads[W], grads[b] = affine_relu_backward(dout, caches.pop())
            elif self.normalization == 'batchnorm':
                dout, grads[W], grads[b], grads[gamma], grads[beta] = affine_bn_relu_backward(dout, caches.pop())
            elif self.normalization == 'layernorm':
                dout, grads[W], grads[b], grads[gamma], grads[beta] = affine_ln_relu_backward(dout, caches.pop())
