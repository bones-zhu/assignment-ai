...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        pad = (filter_size - 1) // 2
        h_mid = (input_dim[1] + 2 * pad - filter_size) // 1 + 1
        w_mid = (input_dim[2] + 2 * pad - filter_size) // 1 + 1
        h_out = (h_mid - 2) // 2 + 1
        w_out = (w_mid - 2) // 2 + 1
        
        self.params['W1'] = np.random.normal(scale=weight_scale, size=(num_filters, input_dim[0], filter_size, filter_size))
        self.params['b1'] = np.zeros(num_filters)
        self.params['W2'] = np.random.normal(scale=weight_scale, size=(num_filters * h_out * w_out, hidden_dim))
